...
(function () {
    ...
    USE(function (module) {
        ...
        SEA.encrypt = SEA.encrypt || (async (data, pair, cb, opt) => {
            try {
                ...
                var ct = await aeskey(key, rand.s, opt).then((aes) => ( /*shim.ossl ||*/shim.subtle).encrypt({
        ...
        SEA.decrypt = SEA.decrypt || (async (data, pair, cb, opt) => {
            try {
                ...
                try {
                    ...
                    var ct = await aeskey(key, buf, opt).then((aes) => ( /*shim.ossl ||*/shim.subtle).decrypt({
            ...
            try {
                ...
                */
                ...
                certificants = (() => {
                    var data = [];
                    if (certificants) {
                        if ((typeof certificants === 'string' || Array.isArray(certificants)) && certificants.indexOf('*'))
                            return '*';
                ...
                const data = JSON.stringify(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({ c: certificants }, (expiry ? { e: expiry } : {})), (readPolicy ? { r: readPolicy } : {})), (writePolicy ? { w: writePolicy } : {})), (readBlacklist ? { rb: readBlacklist } : {})), (writeBlacklist ? { wb: writeBlacklist } : {})));
    ...
    USE(function (module) {
        ...
        check.pub = function (eve, msg, val, key, soul, at, no, user, pub) {
                        ...
                        if (u !== data && data.c && data.w && (data.c === certificant || data.c.indexOf('*' || certificant) > -1)) {
                            ...
                            for (const lex of w) {
                                if ((String.match(path, lex['#']) && String.match(key, lex['.'])) || (!lex['.'] && String.match(path, lex['#'])) || (!lex['#'] && String.match(key, lex['.'])) || String.match((path ? path + '/' + key : key), lex['#'] || lex)) {
                                    ...
                                    if (lex['+'] && lex['+'].indexOf('*') > -1 && path && path.indexOf(certificant) == -1 && key.indexOf(certificant) == -1)
                                        return no(`Path "${path}" or key "${key}" must contain string "${certificant}".`);
            ...
            if ('pub' === key && '~' + pub === soul) {
                if (val === pub)
                    return eve.to.next(msg); // the account MUST match `pub` property that equals the ID of the public key.
        ...
        function security(msg) {
            ...
            if (msg.get) {
                ...
                if (soul) { // for now, only allow direct IDs to be read.
                    ...
                    else if ('~@' === soul.slice(0, 2)) { // Allow reading the list of public keys associated with an alias?
                        return to.next(msg); // yes.
                    }
